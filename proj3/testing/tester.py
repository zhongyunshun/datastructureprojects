...
The instructions each have one of the following forms:
   ...
   C DIR  Create, if necessary, and switch to a subdirectory named DIR under
          the main directory for this base.  If DIR is missing, changes
          back to the default directory.  This command is principally
          intended to let you set up remote repositories.
   T N    Set the timeout for gitlet commands in the rest of this base to N
          seconds.
   + NAME F
...
def reportDetails(base, included_files, line_num):
    if show is None:
        return
    if show <= 0:
    ...
    print("    Error on line {} of {}".format(line_num, basename(base)))

    for base in [basename(base)] + included_files:
        full = join(dirname(base), base)
...
def line_reader(f, prefix):
    ...
    try:
        ...
        if not os.path.exists(filepath):
            filepath = 'samples/' + f + '.in'
        if not os.path.exists(f):
            filepath = 'gitlet_run/' + f + '.in'
    ...
    try:
        ...
        while True:
            ...
            if Match(r'\s*#', line) or Match(r'\s+$', line):
                ...
                inp.send(join(dirname(base), Group(1)))
                ...
                if msg != "OK":
                    ...
                    reportDetails(base, included_files, line_num)
                    return False
            elif Match(r'=\s*(\S+)\s+(\S+)', line):
                ...
                raise ValueError("bad base line at {}".format(line_num))
    finally:
        if not keep:
            cleanTempDir(tmpdir)
